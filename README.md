...
- **Background Processing Queue**: WP-Cron queue processing for automatic regeneration
...
### MVDB Setup (Optional)
1. Configure a GraphQL endpoint that supports a `similarity(query: ...)` query
2. Enter your MVDB endpoint URL
3. Add API key if required
4. Enable MVDB in plugin settings
...
- For best results in production, still ship fully bundled block assets (including `build/index.asset.php`) from your build pipeline.
...
```
ai-tldr-block/
├── ai-tldr-block.php          # Main plugin bootstrap
├── README.md                  # Project documentation
├── includes/                  # Core PHP classes
│   ├── class-TLDR-Service.php # OpenAI integration & summary logic
│   ├── class-TLDR-Rest.php    # REST API endpoints
│   ├── class-TLDR-Cron.php    # Queue + background processing
│   ├── class-TLDR-Content.php # Content normalization and MVDB fetching
│   └── class-TLDR-Admin.php   # Settings page and admin actions
├── assets/                    # Admin-only CSS/JS
│   ├── css/admin.css
│   └── js/admin.js
├── admin/views/               # Settings page view
│   └── settings.php
├── src/block/                 # Block source files
│   ├── block.json             # Block metadata
│   ├── index.js               # Block registration entry
│   ├── edit.js                # Editor interface
│   ├── save.js                # Save function (dynamic block placeholder)
│   ├── style.scss             # Frontend styles
│   └── editor.scss            # Editor styles
└── build/                     # Committed build/runtime assets
    ├── block.json
    ├── index.js
    ├── edit.js
    ├── save.js
    ├── editor-fallback.js     # Fallback editor script when asset manifest is missing
    ├── style.css
    ├── editor.css
    ├── render.php             # Server-side rendering template
    └── frontend.js            # Expand/collapse behavior
```
...
- `_ai_tldr_auto_regen`: Per-post auto-regeneration flag
...
- `GET /wp-json/ai-tldr/v1/test-openai` - Test OpenAI connectivity

### Hooks & Filters
This plugin currently does **not** expose custom `tldr_*` extension hooks.

Internally, it relies on standard WordPress hooks such as `init`, `rest_api_init`, `save_post`, and `cron_schedules`.
...
- **MVDB GraphQL Endpoint**: Optional, for vector-grounded summaries
...
- Verify MVDB is enabled in plugin settings
- Check MVDB endpoint URL and API key
- Confirm the endpoint supports the expected GraphQL similarity query
...
This repository currently does not include a Node build toolchain (`package.json`).
Keep `build/` synchronized manually when editing `src/`:

```bash
# Copy JavaScript files
cp src/block/index.js build/
